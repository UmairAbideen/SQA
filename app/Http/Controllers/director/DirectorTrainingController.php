...
class DirectorTrainingController extends Controller
{
    public function view(Request $request)
    {
        ...
        if ($request->filled('name')) {
            $query->whereHas(
                'user',
                fn($q) => $q->where('username', 'like', '%' . $request->name . '%')
            );
        }
        ...
        if ($request->filled('ses_no')) {
            $query->whereHas(
                'user',
                fn($q) => $q->where('ses_no', 'like', '%' . $request->ses_no . '%')
            );
        }
        ...
        if ($request->filled('org')) {
            $query->whereHas(
                'user',
                fn($q) => $q->where('org', 'like', '%' . $request->org . '%')
            );
        }
        ...
        return view('director.training.view', compact('allStaff'));
    }

    public function form()
    {
        // Include 'org' column for dropdowns
        $users = User::select('id', 'username', 'ses_no', 'org')->get();
        return view('director.training.add', compact('users'));
    }
    ...
    public function edit($id)
    {
        ...
        $users = User::all(); // Optional: allow changing user
    ...
    public function delete($id)
    {
        ...
        return redirect()->route('director.training.view')->with('status', 'Staff authorization deleted successfully.');
    }
